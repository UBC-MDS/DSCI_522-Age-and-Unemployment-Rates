...
The list of countries used from the dataset can be found in Appendix A1. One item to note is that male and female unemployment rates for each country and year are taken as 2 separate observations and are viewed as equally important. 
...
```{r echo=FALSE, out.width='70%', fig.width=7,fig.height=6}
knitr::include_graphics('../img/mean-CI.png')
```
...
## Assumptions
In order to apply the ANOVA, we are assuming that the following conditions are satisified. First of all, all of our samples are independent of each other. Secondly, the variances between different age groups are assumed to be equal. Thirdly, to be able to use anova and pairwise t-tests, we assume our data and residuals are normally distributed. 
...
(section: World Bank high-income economies)
-    Dataset
